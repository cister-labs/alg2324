...
\setLecture{5}{Amortised analysis of algorithms [WiP]}
...
\begin{itemize}
    ...
    \item \texttt{dequeue} \emph{pops} a value from stack \texttt{B} if non-empty otherwise \emph{pops} all elements from stack \texttt{A} and \emph{pushes} them to stack \texttt{B}, and \emph{pops} an element from \texttt{A} that returns;
