...
\begin{frame}{Assessment}
  ...
  \item \textbf{70\%} (\structure{IE}) -- an \structure{improvement exam} that can replace the final exam (if taken);
  \end{itemize}

  There will be 2 exam periods:
  \begin{itemize}
    \item Normal period:
      \mycbox[0.6]{$max(\alert{FE},\structure{IE})*0.7+ \alert{IT}*0.3 ~~ (\geq 9.5)$}
