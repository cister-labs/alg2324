...
\begin{slide}{Syllabus}
  \frsplitdiff{0.52}{0.45}{
    \begin{itemize}
      ...
      \item Fundamentals of asymptotic analysis
...
\begin{frame}{Assessment}
  ...
  \item \textbf{80\%} (\structure{IE}) -- an \structure{improvement exam} that can replace the final exam (if taken);
  \end{itemize}

  There will be 2 exam periods:
  \begin{itemize}
    \item Normal period:
      \mycbox[0.6]{$max(\alert{FE},\structure{IE})*0.8+ IT*0.2 ~~ (\geq 9.5)$}
    \item Extra period (\emph{recurso}): 
      \mycbox[0.6]{$max(\alert{FE}*0.8+IT*0.2, \alert{FE}) ~~ (\geq 9.5)$}
  \end{itemize}
\end{frame}
